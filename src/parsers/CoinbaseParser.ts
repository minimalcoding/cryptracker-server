...
export default class CoinbaseParser {
    parse(rows: string[][]): Transaction[] {
        const toDecimal = v => new Decimal(R.isEmpty(v) ? 0 : v);

        if (R.pipe(R.head, R.head)(rows) !== 'Transactions') {
            throw new Error('This is not a proper Coinbase Transactions export file.');
        }

        return R.pipe(
            // Five first lines are just useless metadata.
            R.drop(5),
            R.reduce((acc: Transaction[], row: string[]): Transaction[] => {
